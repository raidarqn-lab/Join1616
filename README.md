...
## 1. Test the design
Open `index.html` in a browser to test the application. Open `story.html` to test the standalone 1616 History page. Keep the HTML, CSS, JavaScript, and image files together in the same folder.
...
Upload the complete folder, including `index.html`, `story.html`, `styles.css`, `story.js`, `story-translations.js`, and the image assets, then enable GitHub Pages under:
Settings → Pages → Deploy from a branch.

The application and Story page use relative links, so both work from the same GitHub Pages directory.
...
## v96 — separate Application and Story pages
- Moved 1616 History to the standalone `story.html` page.
- Kept the five-step transfer application in `index.html`.
- Extracted the shared visual styles to `styles.css`.
- The application force-saves its draft before opening History.
- Form fields, radio choices, select values, group members, language, and current step restore when the applicant returns.
- Changing the language on the Story page updates only the saved language and leaves all application data intact.
